...
- 2026-02-27: Fixed API `No sub claim` failures by accepting either raw `sub` or mapped `ClaimTypes.NameIdentifier` in `GetKeycloakId`, preserving strict failure when neither exists. This handles Keycloak JWT claim mapping differences in ASP.NET without broad fallbacks. Files: `CampLog.Api\Extensions\ClaimsPrincipalExtensions.cs`, `CampLog.Tests\UnitTest1.cs`.
