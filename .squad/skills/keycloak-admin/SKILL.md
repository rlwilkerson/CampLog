...
## CampLog Project — Creating Users via Admin REST API (PowerShell)

The admin password is auto-generated and stored in AppHost user secrets at `Parameters:keycloak-password`.
The admin username is `admin`. Keycloak runs at `http://localhost:8080`. The realm is `camplog`.

**Step 1 — Get the admin password from secrets:**

```powershell
$secretsFile = "C:\Users\{username}\AppData\Roaming\Microsoft\UserSecrets\{UserSecretsId}\secrets.json"
# Or read UserSecretsId from CampLog.AppHost.csproj and construct the path dynamically:
$proj = "C:\code\rlwilkerson\CampLog\main\CampLog.AppHost\CampLog.AppHost.csproj"
$id = (Select-String -Path $proj 'UserSecretsId' | Select-Object -First 1).Line -replace '.*<UserSecretsId>(.*)</UserSecretsId>.*','$1'
$pass = (Get-Content "$env:APPDATA\Microsoft\UserSecrets\$id\secrets.json" | ConvertFrom-Json).'Parameters:keycloak-password'
```

**Step 2 — Get an admin token (URL-encode the password first):**

```powershell
Add-Type -AssemblyName System.Web
$encodedPass = [System.Web.HttpUtility]::UrlEncode($pass)
$body = "client_id=admin-cli&username=admin&password=$encodedPass&grant_type=password"
...
Invoke-RestMethod -Uri "http://localhost:8080/admin/realms/camplog/users" `
    -Method Post -Headers $headers -Body $newUser
```

**Step 4 — Assign a realm role:**

```powershell
# Get the new user's ID
$userId = (Invoke-RestMethod -Uri "http://localhost:8080/admin/realms/camplog/users?username=testuser" -Headers $headers)[0].id

# Get the role definition, then POST it as an array
$roleInfo  = Invoke-RestMethod -Uri "http://localhost:8080/admin/realms/camplog/roles/user" -Headers $headers
$roleBody  = ConvertTo-Json @($roleInfo) -Depth 3
Invoke-RestMethod -Uri "http://localhost:8080/admin/realms/camplog/users/$userId/role-mappings/realm" `
    -Method Post -Headers $headers -Body $roleBody
```

**Notes:**
- The password must be URL-encoded before embedding in the form body — special characters (e.g. `+`, `.`) will break authentication otherwise.
- Role assignment requires POSTing an **array** (`@($roleInfo)`), not a single object.
- Users added this way persist because Keycloak uses `WithDataVolume()` in AppHost.cs.
- The realm import (`camplog-realm.json`) also defines `testuser` so the user is recreated if the volume is ever wiped.
